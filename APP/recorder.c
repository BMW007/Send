...
void recoder_wav_init(__WaveHeader* wavhead) //初始化WAV头			   
{
 ...
 //	wavhead->fmt.ByteRate=wavhead->fmt.SampleRate;//8位,即1个字节
...
{  
	LCD_ShowString(60+110,250,200,16,16,"AGC:    ");	  	//显示名称,同时清除上次的显示	  
...
u8 recoder_play(void)
{
	...
	u32 recsec=0;					//录音时间
 	u8 recagc=3;					//默认增益为3
	 recbuf=mymalloc(SRAMIN,512); 	//recbuf=mymalloc(SRAMIN,512); 
	 if(recbuf==NULL)
		 rval=1;	  		   
 	if(rval==0)									//内存申请OK
	{      
			...
			if(rec_sta==0X80)//已经在录音了
			{
		  	w=VS_RD_Reg(SPI_HDAT1);	
				if((w>=256)&&(w<896))
				{
					...
					USART_SendStr(recbuf,512); //USART_SendStr(recbuf,512);
					sectorsize++;	
				}			
			}
/////////////////////////////////////////////////////////////
